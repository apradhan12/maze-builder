...
class Maze:
    ...
    def __init__(self, width, height):
        ...
        self.set_cell(self.path[-1], self.WHITE)
        ...
        while self.path:
            while True:
                ...
                if not valid_directions:
                    break
                self.path.append(np.copy(self.path[-1]) + random.choice(valid_directions))
                self.set_cell(self.path[-1], self.WHITE)
            while self.path and not list(filter(self.valid_dir, self.DIRECTIONS)):
                del self.path[-1]
                if self.path:
                    self.set_cell(self.path[-1], self.RED)

    def has_move(self):
        return any(map(self.valid_dir, self.DIRECTIONS))

    def valid_dir(self, direction):
        new_pos = self.path[-1] + direction
        filtered_neighbors = list(filter(lambda pos: self.get_cell(pos) != self.BLACK, self.neighbors(new_pos)))
        return (self.valid_pos(new_pos)
                and self.get_cell(new_pos) == self.BLACK
                and np.array_equal(filtered_neighbors, [self.path[-1]]))
